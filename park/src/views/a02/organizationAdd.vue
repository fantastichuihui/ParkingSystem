...
export default {
  ...
  created() {
    if(this.$route.params.organization){
      this.organization.pcode = this.$route.params.organization.code
      this.organization.pname = this.$route.params.organization.name
    }
  },
  methods: {
    handleYes() {
       this.$refs['organization'].validate((valid) => {
        if (valid) {
          ...
          axios.post('/api/a02person/organization/insertorg', this.organization).then((res) => {
            if (res.data > 0) {
              systemTips.success('添加成功')
              this.$router.push('organization')
            } else {
              systemTips.error('添加失败')
            }
          }).catch(e => {
              systemTips.error('添加失败')
          });
    ...
    handleReset() {
        ...
        this.organization.orgLevel= 1
        this.organization.sort= 1
    }
  }
}
</script>
