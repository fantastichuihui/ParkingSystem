      ...
      <el-row type="flex" justify="center">
        <el-col :offset="7" :xs="2" :sm="4" :md="6" :lg="8" :xl="10">
          <el-form-item label="存储费用：" prop="cost">
            <el-input v-model.number="materielCost.cost" class="g_elFormInput_other" placeholder="请输入存储费用" clearable />
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  data() {
    return {
      labelPosition: 'right',
      materielCost: {
        id: this.$route.params.materielCost.id,
        categoryId: this.$route.params.materielCost.categoryId,
        matNo: this.$route.params.materielCost.matNo,
        name: this.$route.params.materielCost.name,
        contractId: this.$route.params.materielCost.contractId,
        contractItemId: this.$route.params.materielCost.contractItemId,
        houseId: this.$route.params.materielCost.houseId,
        positionId: this.$route.params.materielCost.positionId,
        stockStatus: this.$route.params.materielCost.stockStatus,
        limitStatus: this.$route.params.materielCost.limitStatus,
        inDate: this.$route.params.materielCost.inDate,
        bStrawMat: this.$route.params.materielCost.bStrawMat,
        mStrawMat: this.$route.params.materielCost.mStrawMat,
        sStrawMat: this.$route.params.materielCost.sStrawMat,
        freeTerm: this.$route.params.materielCost.freeTerm,
        cost: this.$route.params.materielCost.cost,
        weight: this.$route.params.materielCost.weight,
        length: this.$route.params.materielCost.length,
        width: this.$route.params.materielCost.width,
        height: this.$route.params.materielCost.height,
        stand: this.$route.params.materielCost.stand,
        proId: this.$route.params.materielCost.proId,
        contractNumber: this.$route.params.materielCost.contractNumber,
        costId: this.$route.params.materielCost.costId,
  ...
  methods: {
    ...
    changeStatus1() {
      ...
      this.materielCost.contractItemId = ''
    },
    ...
    handleYes(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('/api/b04wms/materiel/updateMateriel', this.materielCost).then(res => {
            this.$notify({
              type: 'success',
              title: '提示',
              message: '更新成功!',
