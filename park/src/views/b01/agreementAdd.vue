      ...
      <div v-for="(item,index) in confiles" :key="index">
        <el-form>
          <el-row style="margin-left:20px">
            <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
              附件名称：
              <el-input v-model="item.fileName" class="g_input" placeholder="请输入内容" />
            </el-col>
            <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
              附件类型：
              <el-select v-model="item.typeId" class="g_input" placeholder="请选择类型">
                <el-option v-for="item1 in attcahOptions" :key="item1.value" :label="item1.label" :value="item1.value" />
              </el-select>
            </el-col>
            <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
              <el-input v-if="showing" v-model="item.name" :disabled="true" class="g_input" placeholder="请选择选择按钮" />
              <el-upload
                ref="upload"
                class="upload-demo"
...
export default {
  ...
  methods: {
    ...
    // 查询
    handleSearch() {
      axios.get('/api/b01crm/contract/getUnlockAndLeads').then((result) => {
    ...
    handleRefresh() {
      ...
      this.paymentType = ''
      this.value1 = true
      this.confiles[0].fileName = ''
      this.confiles[0].typeId = ''
      this.confiles.splice(1)
      this.returnPlans[0].money = ''
      this.returnPlans[0].returnDate = ''
      this.returnPlans[0].returnType = ''
      this.returnPlans.splice(1)
    },
    // 添加附件
    attachmentAdd() {
      this.confiles.push({})
    ...
    next() {
      ...
      this.$router.push({ name: 'agreementAddSon', params: { addParams: this.addParams() }})
    },
    // 返回
    handleBack() {
