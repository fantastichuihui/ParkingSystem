<template>
  <div>
    <div class="g_inputForm">
      <el-row style="margin-left:20px;">
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          园区名称：
          <el-select v-model="addParkName" placeholder="请选择园区" class="g_input" @change="inputChange">
            <el-option
              v-for="item in allName"
              :key="item"
              :label="item"
              :value="item"
            />
          </el-select>
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          省份：
          <el-input
            v-model="addProvince"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            disabled
            @keyup.enter.native="handleFilter"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          所在城市：
          <el-input
            v-model="addCity"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            disabled
            @keyup.enter.native="handleFilter"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          联系人姓名：
          <el-input
            v-model="addConcateName"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            disabled
            @keyup.enter.native="handleFilter"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          仓库编码：
          <el-input
            v-model="addStoCode"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            @keyup.enter.native="handleFilter"
            @change="inputChange"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          仓库名称：
          <el-input
            v-model="addStoName"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            @keyup.enter.native="handleFilter"
            @change="inputChange"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          负责人姓名：
          <el-input
            v-model="addchargeName"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            @keyup.enter.native="handleFilter"
            @change="inputChange"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          联系电话：
          <el-input
            v-model="addMobilePhone"
            placeholder="请输入内容"
            class="g_input"
            maxlength="15"
            @keyup.enter.native="handleFilter"
            @change="inputChange"
          />
        </el-col>
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          备注信息：
          <el-input v-model="addTextarea" type="textarea" placeholder="请输入内容" class="g_input" style="margin-bottom: 0; width: 300px" />
        </el-col>

        <div class="g_button">
          <el-button type="primary" plain icon="el-icon-search" @click="addParkReset">重置</el-button>
          <el-button type="primary" plain icon="el-icon-minus" @click="addParkCancel">取消</el-button>
          <el-button type="primary" plain icon="el-icon-refresh" :disabled="buttonLimit" @click="addParkSave">保存</el-button>
        </div>
      </el-row>
    </div>
  </div>
</template>

<script>
export default {
  name: 'TestTable',
  data() {
    return {
      addParkName: '', // 添加园区名称
      addProvince: '', // 添加省份
      addCity: '', // 添加城市
      addConcateName: '', // 添加联系人
      addStoCode: '', // 添加库房编码
      addStoName: '', // 库房名称
      addchargeName: '', // 负责人姓名
      addMobilePhone: '', // 添加负责人联系人电话
      addTextarea: '', // 添加备注
      allParkName: [],
      allPark: [],
      parkId: '',
      buttonLimit: true,
      allName: []
    }
  },

  // 监视园区名称的选择
  watch: {
    addParkName: function() {
      this.allParkName.forEach((item, index) => {
        if (item.name === this.addParkName) {
          this.addProvince = item.Column_15
          this.addCity = item.Column_17
          this.addConcateName = item.concateName
          this.parkId = item.id
        }
      })
    }
  },

  created: function() {
    const list = {}
    list.page = 1
    list.rows = 10
    const array = new Set()
    this.$axios({
      method: 'get',
      url: '/api/b03res/packmanage/findallpark',
      params: list
    }).then(response => {
      this.allParkName = response.data.resultMap.rows
      response.data.resultMap.rows.forEach((item, index) => {
        array.add(item.name)
      })
      this.allName = array
    })
  },

  methods: {
    //   重置
    addParkReset: function() {
      this.addParkName = ''
      this.addProvince = ''
      this.addCity = ''
      this.addConcateName = ''
      this.addStoCode = ''
      this.addStoName = ''
      this.addchargeName = ''
      this.addMobilePhone = ''
      this.addTextarea = ''
    },

    // 保存按钮的限制
    inputChange: function() {
      if (!this.addParkName || !this.addStoCode || !this.addStoName || !this.addchargeName || !this.addMobilePhone) {
        this.buttonLimit = true
      } else {
        this.buttonLimit = false
      }
    },

    // 取消
    addParkCancel: function() {
      this.$router.push({
        name: 'storageroom'
      })
    },

    // 保存
    addParkSave: function() {
      const list = {}
      list.name = this.addStoName
      list.code = this.addStoCode
      list.chargeName = this.addchargeName
      list.remark = this.addTextarea
      list.parkId = this.parkId
      list.chargeId = 2
      list.state = 1
      list.modifierCode = this.$store.state.userInfo.code
      list.createUserCode = 'k001'
      list.modifierName = this.$store.state.userInfo.name
      this.$axios({
        method: 'post',
        url: '/api/b03res/storageroom/addstorageroom',
        data: list
      }).then(response => {
        if (response.data.status === 1) {
          this.$notify({
            title: '成功',
            message: '添加库房信息成功',
            type: 'success',
            position: 'bottom-right'
          })
        }
        this.$router.push({
          name: 'storageroom'
        })
      })
    }
  }
}
</script>
