  ...
  <div>
    <div class="g_inputForm">
      <el-row style="margin-left:20px;">
        ...
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          仓库编码：
          <el-select v-model="temp.s_code" :disabled="true" placeholder="请选择仓库" class="g_input">
            <!-- <el-option
        ...
        <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
          作业区号：
          <el-select v-model="temp.a_id" placeholder="请选择作业号" class="g_input">
            <el-option
              v-for="item in allPark"
              :key="item.a_id"
              :label="item.a_name"
        ...
        <div v-for="(item,index) in context" :key="index">
          ...
          <el-col :xs="2" :sm="4" :md="6" :lg="8" :xl="6">
            备注：
            <el-input v-model="item.remark" placeholder="请输入备注" class="g_input" maxlength="30" />
          </el-col>
        </div>
...
export default {
  ...
  watch: {
    'temp.a_id'(newName, oldName) {
      this.allPark.forEach((item, index) => {
        if (item.a_id === this.temp.a_id) {
          this.temp.a_id = item.a_id
  ...
  created() {
    if (JSON.stringify(this.$route.params) === '{}') {
      this.addflg = true
      axios.get('/api/b03res/crane/getDatas').then(res => {
      ...
      axios.get('/api/b03res/crane/getDatas').then(res => {
        this.allPark = res.data.resultData.datalist
        this.temp.a_id = this.addtemp.a_id
  ...
  methods: {
    editParkSave() {
      if (
        JSON.stringify(this.temp.a_id) === '{}' ||
        this.temp.a_id === undefined || this.temp.a_id === ''
      ) {
        this.$notify({
          title: '提示信息',
          message: '请选择作业区号',
    ...
    addSure() {
      ...
      for (let i = 0; i < this.context.length; i++) {
        ...
        temp.a_id = this.temp.a_id
        temp.number = this.context[i].number
        temp.remark = this.context[i].remark
      ...
      data.list = array
      axios
        .post('/api/b03res/crane/add', data)
        .then(response => {
          // 返回值为0后台操作未成功
    ...
    editSure() {
      ...
      data.remark = this.context[0].remark
      ...
      axios
        .put('/api/b03res/crane/update', data)
        .then(response => {
          // 返回值为0后台操作未成功
    ...
    editParkReset() {
      this.addtemp = Object.assign({}, this.$route.params) // copy obj  将选中的数据放入表格中
      this.temp.a_id = this.addtemp.a_id
      this.temp.a_number = this.addtemp.a_number
      this.temp.a_name = this.addtemp.a_name
      this.temp.p_id = this.addtemp.p_id
      this.temp.p_name = this.addtemp.p_name
      this.temp.p_province = this.addtemp.p_province
      this.temp.p_city = this.addtemp.p_city
